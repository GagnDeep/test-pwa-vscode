...
class ExtensionManager {
	constructor() {
		const extensionFolderUri = vscode_1.Uri.file(
			vscode_1.extensions.getExtension(env_1.MATERIAL_THEME_EXT_ID)
				.extensionPath
		);
		this.configFileUri = extensionFolderUri.with({
			path: path_1.posix.join(extensionFolderUri.path, env_1.CONFIG_FILE_NAME),
		});
		this.userConfigFileUri = extensionFolderUri.with({
			path: path_1.posix.join(
				extensionFolderUri.path,
				env_1.USER_CONFIG_FILE_NAME
			),
		});
	}
	get VERSION_KEY() {
	...
	getPackageJSON() {
		return vscode_1.extensions.getExtension(env_1.MATERIAL_THEME_EXT_ID)
			.packageJSON;
	}
	getConfig() {
	...
	updateConfig(config) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			yield vscode_1.workspace.fs.writeFile(
				this.configFileUri,
				Buffer.from(JSON.stringify(newConfig), 'utf-8')
			);
		});
	}
	init(context) {
		return __awaiter(this, void 0, void 0, function* () {
			try {
				...
				const themeNeverUsed =
					mementoStateVersion === undefined ||
					typeof mementoStateVersion !== 'string';
				...
				if (themeNeverUsed) {
					yield context.globalState.update(
						this.VERSION_KEY,
						packageJSON.version
					);
				}
				const configBuffer = yield vscode_1.workspace.fs.readFile(
					this.configFileUri
				);
				const configContent = Buffer.from(configBuffer).toString('utf8');
				this.configJSON = JSON.parse(configContent);
				const userConfigUpdate = Object.assign(
					Object.assign({}, this.configJSON),
					{ changelog: { lastversion: packageJSON.version } }
				);
				yield vscode_1.workspace.fs.writeFile(
				...
				yield vscode_1.window.showErrorMessage(
					`Material Theme: there was an error while loading the configuration. Please retry or open an issue: ${String(
						error
					)}`
				);
			}
