...
const getThemeColorCustomizationsConfig = (accentColor) => {
	const {
		accentsProperties,
	} = extension_manager_1.extensionManager.getConfig();
	const color = isValidColor(accentColor) ? accentColor : undefined;
	const themeColorCustomConfig = Object.keys(accentsProperties).reduce(
		(acc, propName) => {
			const currentProp = accentsProperties[propName];
			const shouldModify = color && currentProp.alpha < 100;
			const colorProp = shouldModify
				? `${color}${
						currentProp.alpha > 10 ? currentProp.alpha : `0${currentProp.alpha}`
				  }`
				: color;
...
const updateColorCustomizationsConfig = (config) =>
	__awaiter(void 0, void 0, void 0, function* () {
		try {
			yield vscode_1.workspace
				.getConfiguration()
				.update('workbench.colorCustomizations', config, true);
			return true;
		} catch (error) {
...
const command = () =>
	__awaiter(void 0, void 0, void 0, function* () {
		...
		const currentThemeID = vscode_1.workspace
			.getConfiguration()
			.get('workbench.colorTheme');
		const isMaterialTheme = Boolean(
			pkg.contributes.themes.find((theme) => theme.label === currentThemeID)
		);
		if (!isMaterialTheme) {
			return;
		}
		const themeConfig = extension_manager_1.extensionManager.getConfig();
		const currentColorCustomizationsConfig = vscode_1.workspace
			.getConfiguration()
			.get('workbench.colorCustomizations');
		const accent = yield quickPick();
		let config = {};
		if (accent === PURGE_KEY) {
			const _a = currentColorCustomizationsConfig,
				_b = currentThemeID,
				_ = _a[_b],
			...
			config = Object.assign(
				Object.assign({}, currentColorCustomizationsConfig),
				{
					[`[${currentThemeID}]`]: Object.assign(
						{},
