(function () {
		...
		1: [
			function (require, module, exports) {
				...
				const renderTemplate = (posts) => {
					return `${posts.reduce(
						(acc, { version, title, fixed, new: newItems, breaking }) =>
							acc.concat(`<section class="Release">
    ...
    <ul class="Release-List">
      ${fixed.reduce(
				(accc, src) =>
					src.length > 0
						? accc.concat(`<li data-type="fixed">${src}</li>`)
						: '',
				''
			)}
      ${newItems.reduce(
      ...
      ${breaking.reduce(
				(accc, src) =>
					src.length > 0
						? accc.concat(`<li data-type="breaking">${src}</li>`)
						: '',
				''
			)}
    </ul>
				...
				getReleaseNotes().then((res) => {
					const normalized = res.reduce(
						(acc, src) =>
							acc.concat(
								Object.assign(Object.assign({}, src), {
									fixed: src.fixed
										? src.fixed.map((item) => item.children[0].text)
										: [],
									new: src.new
										? src.new.map((item) => item.children[0].text)
										: [],
									breaking: src.breaking
										? src.breaking.map((item) => item.children[0].text)
										: [],
								})
							),
						[]
					);
					document.querySelector('.Container').innerHTML = renderTemplate(
						normalized
					);
				});
			},
					...
					(function () {
						...
						(function (root, factory) {
							...
							if (typeof define === 'function' && define.amd) {
								...
								let evsImportName =
									(root._eventSourceImportPrefix || '') + 'EventSource';
								root[evsImportName] = factory();
							}
						})(typeof self === 'undefined' ? this : self, function () {
							let EventSource = function (url, options) {
							...
							EventSource.prototype = {
								...
								_onxhrdata: function () {
									...
									if (request.isReady() && !request.hasError()) {
										...
										let lastMessageIndex = this.lastMessageIndex(buffer);
										if (lastMessageIndex[0] >= this.cursor) {
											let newcursor = lastMessageIndex[1];
											let toparse = buffer.substring(this.cursor, newcursor);
								...
								parseStream: function (chunk) {
									...
									let i, j, eventType, datas, line, retry;

									for (i = 0; i < events.length - 1; i++) {
										eventType = 'message';
										datas = [];
										let parts = events[i].split('\n');
								...
								dispatchEvent: function (type, event) {
									let handlers = this['_' + type + 'Handlers'];
								...
								lastMessageIndex: function (text) {
									let ln2 = text.lastIndexOf('\n\n');
									let lr2 = text.lastIndexOf('\r\r');
									let lrln2 = text.lastIndexOf('\r\n\r\n');
							...
							if (isOldIE()) {
								...
								let defaults = EventSource.prototype.defaultOptions;
								...
								EventSource.prototype.XHR = function (evs) {
									...
									request.open(
										'GET',
										evs.urlWithParams(evs.URL, evs.getArgs),
										true
									);

									let headers = evs.xhrHeaders; // maybe null
		...
		3: [
			function (require, module, exports) {
				...
				let _require = require('@sanity/observable/operators/map'),
					map = _require.map;

				let _require2 = require('@sanity/observable/operators/filter'),
					filter = _require2.filter;

				let queryString = require('../http/queryString');
				...
				assign(AssetsClient.prototype, {
					...
					upload: function upload(assetType, body) {
						...
						let dataset = validators.hasDataset(this.client.clientConfig);
						let assetEndpoint = assetType === 'image' ? 'images' : 'files';
						let options = optionsFromFile(opts, body);
						...
						let observable = this.client._requestObservable({
							method: 'POST',
					...
					getImageUrl: function getImageUrl(ref, query) {
						...
						validators.hasDataset(this.client.clientConfig);
						let _this$client$clientCo = this.client.clientConfig,
							projectId = _this$client$clientCo.projectId,
							dataset = _this$client$clientCo.dataset;
						let qs = query ? queryString(query) : '';
			...
			},
			{
				'../http/queryString': 14,
				'../validators': 24,
				'@sanity/observable/operators/filter': 29,
				'@sanity/observable/operators/map': 30,
				'object-assign': 66,
			},
		],
		4: [
		...
		5: [
			function (require, module, exports) {
				...
				let generateHelpUrl = require('@sanity/generate-help-url');

				let assign = require('object-assign');

				let validate = require('./validators');

				let once = require('./util/once');

				let defaultCdnHost = 'apicdn.sanity.io';
				let defaultConfig = {
				...
				let isLocal = function isLocal(host) {
					return LOCALHOSTS.indexOf(host) !== -1;
				}; // eslint-disable-next-line no-console

				let createWarningPrinter = function createWarningPrinter(message) {
					return once(function () {
						return console.warn(message.join(' '));
					});
				};

				let printCdnWarning = createWarningPrinter([
				...
				let printBrowserTokenWarning = createWarningPrinter([
				...
				let printCdnTokenWarning = createWarningPrinter([
				...
				exports.initConfig = function (config, prevConfig) {
					let newConfig = assign({}, defaultConfig, prevConfig, config);
					let gradientMode = newConfig.gradientMode;
					let projectBased = !gradientMode && newConfig.useProjectHostname;
					...
					let isLocalhost = isBrowser && isLocal(window.location.hostname);
					...
					newConfig.useCdn =
						Boolean(newConfig.useCdn) &&
						!newConfig.token &&
						!newConfig.withCredentials;

					if (newConfig.gradientMode) {
						newConfig.url = newConfig.apiHost;
						newConfig.cdnUrl = newConfig.apiHost;
					} else {
						let hostParts = newConfig.apiHost.split('://', 2);
						let protocol = hostParts[0];
						let host = hostParts[1];
						let cdnHost = newConfig.isDefaultApi ? defaultCdnHost : host;
		...
		6: [
			function (require, module, exports) {
				...
				let _require = require('@sanity/observable/operators/filter'),
					filter = _require.filter;

				let _require2 = require('@sanity/observable/operators/map'),
					map = _require2.map;
				...
				let encodeQueryString = require('./encodeQueryString');
				...
				let excludeFalsey = function excludeFalsey(param, defValue) {
					let value = typeof param === 'undefined' ? defValue : param;
					return param === false ? undefined : value;
				};

				let getMutationQuery = function getMutationQuery() {
					let options =
				...
				let isResponse = function isResponse(event) {
					return event.type === 'response';
				};

				let getBody = function getBody(event) {
					return event.body;
				};

				let indexBy = function indexBy(docs, attr) {
				...
				let toPromise = function toPromise(observable) {
					return observable.toPromise();
				};

				let getQuerySizeLimit = 11264;
				module.exports = {
					listen: listen,
					getDataUrl: function getDataUrl(operation, path) {
						let config = this.clientConfig;
						let catalog = config.gradientMode
							? config.namespace
							: validators.hasDataset(config);
						let baseUri = '/'.concat(operation, '/').concat(catalog);
						let uri = path ? ''.concat(baseUri, '/').concat(path) : baseUri;
					...
					fetch: function fetch(query, params) {
						...
						let observable = this._dataRequest(
							'query',
							{
								query: query,
								params: params,
					...
					getDocument: function getDocument(id) {
						...
						let observable = this._requestObservable(options).pipe(
							filter(isResponse),
							map(function (event) {
					...
					mutate: function mutate(mutations, options) {
						let mut =
							mutations instanceof Patch || mutations instanceof Transaction
								? mutations.serialize()
								: mutations;
						let muts = Array.isArray(mut) ? mut : [mut];
						let transactionId = options && options.transactionId;
						return this.dataRequest(
							'mutate',
							{
								mutations: muts,
					...
					dataRequest: function dataRequest(endpoint, body) {
						...
						let request = this._dataRequest(endpoint, body, options);
					...
					_dataRequest: function _dataRequest(endpoint, body) {
						let options =
							arguments.length > 2 && arguments[2] !== undefined
								? arguments[2]
								: {};
						let isMutation = endpoint === 'mutate'; // Check if the query string is within a configured threshold,
						// in which case we can use GET. Otherwise, use POST.

						let strQuery = !isMutation && encodeQueryString(body);
						let useGet = !isMutation && strQuery.length < getQuerySizeLimit;
						let stringQuery = useGet ? strQuery : '';
						let returnFirst = options.returnFirst;
						let timeout = options.timeout,
							token = options.token;
						let uri = this.getDataUrl(endpoint, stringQuery);
						let reqOptions = {
					...
					_create: function _create(doc, op) {
						...
						let mutation = _defineProperty({}, op, doc);
		...
		7: [
			function (require, module, exports) {
				...
				module.exports = function (_ref) {
					...
					let base = '?query='.concat(enc(query));
					let qString = Object.keys(params).reduce(function (qs, param) {
						return ''
							.concat(qs, '&')
		...
		8: [
			function (require, module, exports) {
				...
				let Observable = require('@sanity/observable/minimal');

				let polyfilledEventSource = require('@sanity/eventsource');
				...
				let printTokenWarning = once(function () {
					return console.warn(tokenWarning.join(' '));
				});
				let isWindowEventSource = Boolean(
					typeof window !== 'undefined' && window.EventSource
				);
				let EventSource = isWindowEventSource
				...
				module.exports = function listen(query, params) {
					let opts =
						arguments.length > 2 && arguments[2] !== undefined
							? arguments[2]
							: {};
					let options = defaults(opts, defaultOptions);
					let listenOpts = pick(options, possibleOptions);
					let qs = encodeQueryString({
						query: query,
						params: params,
						options: listenOpts,
					});
					let _this$clientConfig = this.clientConfig,
						url = _this$clientConfig.url,
						token = _this$clientConfig.token,
						withCredentials = _this$clientConfig.withCredentials;
					let uri = ''.concat(url).concat(this.getDataUrl('listen', qs));
					let listenFor = options.events ? options.events : ['mutation'];
					let shouldEmitReconnect = listenFor.indexOf('reconnect') !== -1;
				...
				function cooerceError(err) {
					...
					return evt instanceof Error
						? evt
						: new Error(extractErrorMessage(evt));
				}
			...
			},
			{
				...
				'../util/pick': 23,
				'./encodeQueryString': 7,
				'@sanity/eventsource': 25,
				'@sanity/generate-help-url': 26,
				'@sanity/observable/minimal': 28,
				'object-assign': 66,
			},
		],
		9: [
			function (require, module, exports) {
				...
				let validateObject = validate.validateObject;
				let validateInsert = validate.validateInsert;
				...
				assign(Patch.prototype, {
					...
					splice: function splice(selector, start, deleteCount, items) {
						...
						let delAll =
							typeof deleteCount === 'undefined' || deleteCount === -1;
						let startIndex = start < 0 ? start - 1 : start;
						let delCount = delAll ? -1 : Math.max(0, start + deleteCount);
						let delRange = startIndex < 0 && delCount >= 0 ? '' : delCount;
						let rangeSelector = ''
							.concat(selector, '[')
							.concat(startIndex, ':')
							.concat(delRange, ']');
					...
					commit: function commit() {
						...
						let returnFirst = typeof this.selection === 'string';
						let opts = assign(
							{
		...
		10: [
			function (require, module, exports) {
				...
				function Transaction() {
					...
					let transactionId = arguments.length > 2 ? arguments[2] : undefined;
				...
				assign(Transaction.prototype, {
					...
					patch: function patch(documentId, patchOps) {
						...
						let isPatch = documentId instanceof Patch; // transaction.patch(client.patch('documentId').inc({visits: 1}))
						...
						if (isBuilder) {
							let patch = patchOps(new Patch(documentId, {}, this.client));
			...
			{ '../validators': 24, './patch': 9, 'object-assign': 66 },
		],
		11: [
			function (require, module, exports) {
				'use strict';
			...
			{ '../validators': 24, 'object-assign': 66 },
		],
		12: [
			function (require, module, exports) {
				'use strict';
		...
		13: [
			...
			{ 'make-error': 64, 'object-assign': 66 },
		],
		14: [
			function (require, module, exports) {
				'use strict';
		...
		15: [
			function (require, module, exports) {
				...
				let getIt = require('get-it');

				let assign = require('object-assign');

				let observable = require('get-it/lib/middleware/observable');

				let jsonRequest = require('get-it/lib/middleware/jsonRequest');

				let jsonResponse = require('get-it/lib/middleware/jsonResponse');

				let progress = require('get-it/lib/middleware/progress');

				let Observable = require('@sanity/observable/minimal');

				let _require = require('./errors'),
				...
				let envSpecific = require('./nodeMiddleware');

				let middleware = envSpecific.concat([
					jsonRequest(),
					jsonResponse(),
					progress(),
					httpError,
			...
			},
			{
				...
				'@sanity/observable/minimal': 28,
				'get-it': 46,
				'get-it/lib/middleware/jsonRequest': 50,
				'get-it/lib/middleware/jsonResponse': 51,
				'get-it/lib/middleware/observable': 52,
				'get-it/lib/middleware/progress': 54,
				'object-assign': 66,
			},
		],
		16: [
			function (require, module, exports) {
				...
				let projectHeader = 'X-Sanity-Project-ID';
		...
		18: [
			function (require, module, exports) {
				...
				let Patch = require('./data/patch');

				let Transaction = require('./data/transaction');

				let dataMethods = require('./data/dataMethods');

				let DatasetsClient = require('./datasets/datasetsClient');

				let ProjectsClient = require('./projects/projectsClient');

				let AssetsClient = require('./assets/assetsClient');

				let UsersClient = require('./users/usersClient');

				let AuthClient = require('./auth/authClient');

				let httpRequest = require('./http/request');

				let getRequestOptions = require('./http/requestOptions');

				let _require3 = require('./config'),
				...
				function SanityClient() {
					...
					if (this.clientConfig.isPromiseAPI) {
						let observableConfig = assign({}, this.clientConfig, {
							isPromiseAPI: false,
						});
				...
				assign(SanityClient.prototype, {
					...
					config: function config(newConfig) {
						...
						if (this.observable) {
							let observableConfig = assign({}, newConfig, {
								isPromiseAPI: false,
							});
					...
					_requestObservable: function _requestObservable(options) {
						...
						let canUseCdn =
							...
							uri.indexOf('/data/') === 0;
						let reqOptions = getRequestOptions(
							this.clientConfig,
							assign({}, options, {
			...
			{
				'./assets/assetsClient': 3,
				'./auth/authClient': 4,
				'./config': 5,
				'./data/dataMethods': 6,
				'./data/patch': 9,
				'./data/transaction': 10,
				'./datasets/datasetsClient': 11,
				'./http/request': 15,
				'./http/requestOptions': 16,
				'./projects/projectsClient': 17,
				'./users/usersClient': 19,
				'@sanity/observable/operators/filter': 29,
				'@sanity/observable/operators/map': 30,
				'object-assign': 66,
			},
		...
		24: [
			function (require, module, exports) {
				...
				let VALID_ASSET_TYPES = ['image', 'file'];
				let VALID_INSERT_LOCATIONS = ['before', 'after', 'replace'];
				...
				exports.validateInsert = function (at, selector, items) {
					...
					if (VALID_INSERT_LOCATIONS.indexOf(at) === -1) {
						let valid = VALID_INSERT_LOCATIONS.map(function (loc) {
							return '"'.concat(loc, '"');
						}).join(', ');
						throw new Error(
							''
		...
		25: [
			...
			{ '@rexxars/eventsource-polyfill': 2 },
		],
		26: [
			function (require, module, exports) {
...
*/
		...
		35: [
			...
			{
				'./GetIntrinsic': 34,
				'./helpers/assertRecord': 36,
				'./helpers/isFinite': 37,
				'./helpers/isNaN': 38,
				'./helpers/mod': 39,
				'./helpers/sign': 40,
				'es-to-primitive/es5': 41,
				has: 59,
				'is-callable': 60,
			},
		],
		...
		41: [
			...
			{ './helpers/isPrimitive': 42, 'is-callable': 60 },
		],
		42: [
			function (require, module, exports) {
		...
		47: [
			function (require, module, exports) {
				...
				module.exports = function createRequester() {
					...
					var middleware = middlehooks.reduce(
						function (ware, name) {
							ware[name] = ware[name] || [];
							return ware;
						},
			...
			{
				'./middleware/defaultOptionsProcessor': 48,
				'./middleware/defaultOptionsValidator': 49,
				'./request': 56,
				'./util/middlewareReducer': 58,
				'nano-pubsub': 65,
			},
		],
		48: [
			function (require, module, exports) {
				...
				module.exports = function (opts) {
					...
					if (options.query) {
						url.query = objectAssign(
							{},
							url.query,
							removeUndefined(options.query)
						);
					}
			...
			{ 'object-assign': 66, 'url-parse': 103 },
		],
		49: [
			function (require, module, exports) {
				'use strict';
		...
		50: [
						...
						processOptions: function processOptions(options) {
							...
							var shouldSerialize =
								...
								(serializeTypes.indexOf(
									typeof body === 'undefined' ? 'undefined' : _typeof(body)
								) !== -1 ||
									Array.isArray(body) ||
									isPlainObject(body));
			...
			{ 'is-plain-object': 62, 'object-assign': 66 },
		],
		51: [
			function (require, module, exports) {
				'use strict';
				...
				function tryParse(body) {
					try {
						...
						err.message =
							'Failed to parsed response body as JSON: ' + err.message;
						throw err;
					}
				}
			},
		...
		52: [
							...
							return new Observable(function (observer) {
								...
								channels.progress.subscribe(function (event) {
									return observer.next(
										objectAssign({ type: 'progress' }, event)
									);
								});
			...
			{ '../util/global': 57, 'object-assign': 66 },
		],
		53: [
			function (require, module, exports) {
				'use strict';
		...
		55: [
			function (require, module, exports) {
				...
				module.exports = function (context, callback) {
					...
					var cors =
						win && win.location && !sameOrigin(win.location.href, options.url);

					// Allow middleware to inject a response, for instance in the case of caching or mocking
					var injectedResponse = context.applyMiddleware(
						'interceptRequest',
						undefined,
						{
							adapter: adapter,
					...
					function onError() {
						...
						var err = new Error(
							'Network error while attempting to reach ' + options.url
						);
					...
					function reduceResponse() {
						...
						if (isXdr && statusCode === undefined) {
							...
							statusMessage =
								xhr.status === 1223 ? 'No Content' : statusMessage;
						}
			...
			{ 'parse-headers': 70, 'same-origin': 97 },
		],
		56: [
			function (require, module, exports) {
				'use strict';
...
*/
		...
		68: [
			...
			{ './implementation': 67, './isArguments': 69 },
		],
		69: [
			function (require, module, exports) {
				'use strict';
		...
		70: [
			...
			{ 'for-each': 43, 'string.prototype.trim': 100 },
		],
		71: [
			function (require, module, exports) {
				'use strict';
		...
		73: [
			...
			{
				'./config': 77,
				'./symbol/observable': 85,
				'./util/canReportError': 89,
				'./util/pipe': 95,
				'./util/toSubscriber': 96,
			},
		],
		74: [
			function (require, module, exports) {
			...
			{ './config': 77, './util/hostReportError': 90 },
		],
		75: [
			function (require, module, exports) {
				'use strict';
			...
			{
				'../internal/symbol/rxSubscriber': 86,
				'./Observer': 74,
				'./Subscription': 76,
				'./config': 77,
				'./util/hostReportError': 90,
				'./util/isFunction': 92,
			},
		],
		76: [
		...
		84: [
			...
			},
			{
				'../Subscriber': 75,
				'../observable/empty': 78,
				'../util/ArgumentOutOfRangeError': 87,
			},
		],
		85: [
			function (require, module, exports) {
		...
		96: [
			...
			{ '../Observer': 74, '../Subscriber': 75, '../symbol/rxSubscriber': 86 },
		],
		97: [
			function (require, module, exports) {
				'use strict';
				...
				module.exports = function (uri1, uri2, ieMode) {
					...
					var url1Port =
						url1.port | 0 || (url1.protocol === 'https' ? 443 : 80);
					var url2Port =
						url2.port | 0 || (url2.protocol === 'https' ? 443 : 80);
		...
		99: [
			...
			{ 'es-abstract/es5': 35, 'function-bind': 45 },
		],
		100: [
			function (require, module, exports) {
				'use strict';
		...
		102: [
			...
			{ './polyfill': 101, 'define-properties': 33 },
		],
		103: [
			function (require, module, exports) {
				(function (global) {
			...
			{ querystringify: 71, 'requires-port': 72 },
		],
	},
	{},
	[1]
